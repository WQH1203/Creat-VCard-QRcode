...
def clear():
    ...
    img = Image.open('SB-V.png')
    photo_aux_2 = ImageTk.PhotoImage(img)
    lbl_img1['image'] = photo_aux_2
...
def porcura():
    try:
        ...
        messagebox('Error', 'Não tem este id')
        print("Erro:", e)
        mysql_db.rollback()
...
def crear_qrc():
    try:
        ...
        if lbl_Id_V['text'] <= 10:
            ...
            vcard = vcard % (aux[0], aux[1], aux[2], aux[3], aux[4], aux[5], aux[6],
...
btn_Del_Img = Button(root, text='Eleminar imagem', command=dele_img)
...
lbl_cod_postal = Label(fm, text='Codigo Pos. :', font=('Calibri', 20, 'bold italic'), anchor='e', justify='left')
txt_cod_postal = Text(fm)
lbl_pais = Label(fm, text='Pais:', font=('Calibri', 20, 'bold italic'), anchor='e', justify='left')
txt_pais = Text(fm)
...
lbl_cod_postal.place(x=20, y=318, width=145, height=20)
...
lbl_ocup.place(x=35, y=435, width=130, height=20)
